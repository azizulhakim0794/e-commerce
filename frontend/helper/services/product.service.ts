...
export const product_service = {
    ...
    save_product_into_cart: async (data:{quantity: number, product_id:string}): Promise<unknown> => {
    ...
    update_cart_products_quantity: async (data:{quantity:number,cart_id:string}): Promise<unknown> => {
        const { data: responseData } = await api.patch<unknown>(`/cart`,data)
        return responseData
    },

    remove_cart_product: async (cart_id:string): Promise<unknown> => {
        const { data: responseData } = await api.delete<unknown>(`/cart/${cart_id}`)
        return responseData
    }
